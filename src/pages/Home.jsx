      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'0 20px', height:48, background:'var(--bg-card)', borderBottom:'0.5px solid var(--border-default)', position:'sticky', top:0, zIndex:100, width:'100%' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:7, marginLeft:'auto' }}>
          ...
          <button onClick={()=>window.open('https://business.trustdubai.ae','_blank')}
            ...
            <i className="ti ti-briefcase" style={{ fontSize:9 }} /> My Business
          </button>
          {customer ? (
      ...
      <div style={{ background:'var(--bg-primary)', minHeight:'100vh', paddingBottom:72, overflowX:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:'var(--bg-card)', borderBottom:'0.5px solid var(--border-default)', position:'sticky', top:0, zIndex:100 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <ThemeToggle />
            <button onClick={()=>window.open('https://business.trustdubai.ae','_blank')}
              style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', border:'0.5px solid #b3d9f0', borderRadius:99, background:'#f0faff', color:'#0099cc', fontSize:9, fontWeight:600, cursor:'pointer' }}>
              <i className="ti ti-briefcase" style={{ fontSize:9 }} /> My Biz
            </button>
            {customer ? (
              <div style={{ width:22, height:22, borderRadius:'50%', background:'#1a2744', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, color:'#fff' }}>
                {(customer.full_name||customer.email||'U')[0].toUpperCase()}
              ...
              <button onClick={()=>signInWithGoogle()}
                style={{ padding:'4px 8px', border:'none', borderRadius:99, background:'#fff', color:'#374151', fontSize:9, fontWeight:600, cursor:'pointer', boxShadow:'0 1px 4px rgba(0,0,0,0.12)' }}>
                Sign in
              </button>
            )}
          </div>
        ...
        <div style={{ background:'var(--bg-card)', padding:'16px 14px 14px', borderBottom:'0.5px solid var(--border-default)', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', gap:6, justifyContent:'center', marginTop:10 }}>
            {[['companies','Companies'],['reviews','Reviews'],['avgRating','Rating']].map(([k,l])=>(
              <div key={k} style={{ background:'var(--bg-secondary)', border:'0.5px solid var(--border-default)', borderRadius:8, padding:'5px 10px', textAlign:'center' }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#0099cc' }}>{k==='avgRating'?stats.avgRating+'★':fmt(stats[k])}</div>
                <div style={{ fontSize:7, color:'var(--text-muted)', marginTop:1 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <div style={{ background:'var(--bg-card)', border:'0.5px solid var(--border-default)', borderRadius:10, padding:'9px 11px' }}>
                ...
                <div style={{ display:'flex', gap:4, flexWrap:'wrap', marginTop:6 }}>
                  {['Downtown','Business Bay','JBR','DIFC','+8 more'].map(a=>(
                    <span key={a} style={{ fontSize:7.5, background:'#e0f9ff', color:'#0077aa', padding:'2px 6px', borderRadius:4, fontWeight:600 }}>{a}</span>
                  ))}
                </div>
              </div>
            </div>
