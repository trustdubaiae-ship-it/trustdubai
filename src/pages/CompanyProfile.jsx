...
export default function CompanyProfile({ navigate, params }) {
  ...
  if (!company) return <div style={{ padding: 20, textAlign: 'center' }}>Company not found</div>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <button onClick={() => navigate('search', {})} style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: 'var(--text2)' }}>
      ...
      <div style={{
        ...
        background: '#fff', position: 'sticky', top: 57, zIndex: 99
      }}>
        {TABS.map(t => (
      ...
      {tab === 'overview' && (
        <div>
          <div style={{ background: 'var(--bg)', padding: 16, borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>{company.category}</div>
                <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 3 }}>
            ...
            <div style={{
              ...
              padding: '10px 14px', border: '1px solid var(--border)', marginBottom: 10
            }}>
              <div style={{ fontSize: 28, fontWeight: 600 }}>{company.avg_rating || '0.0'}</div>
              <div style={{ flex: 1 }}>
                <Stars rating={company.avg_rating} size={16} />
                <div style={{ fontSize: 12, color: 'var(--text2)' }}>{company.total_reviews || 0} reviews</div>
              </div>
              {company.is_verified && (
                <span style={{ background: 'var(--green-light)', color: 'var(--green)', fontSize: 11, padding: '3px 10px', borderRadius: 10 }}>✓ Verified</span>
              )}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => navigate('add-review', { company })}
                style={{ flex: 1, padding: 9, borderRadius: 20, background: '#fff', color: 'var(--primary)', border: '1.5px solid var(--primary)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                Write Review
              </button>
            </div>
          </div>
          {company.description && (
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
              <p style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6 }}>{company.description}</p>
            </div>
          )}
          <div style={{ padding: '14px 16px 8px', borderBottom: '1px solid var(--border)', fontSize: 14, fontWeight: 500 }}>
            Recent Reviews
          </div>
          {loading ? <p style={{ padding: 20, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>Loading...</p>
          ...
          {reviews.length > 2 && (
            <div style={{ padding: '12px 16px' }}>
              <button onClick={() => setTab('reviews')} style={{
                ...
                borderRadius: 20, background: '#fff', color: 'var(--primary)',
      ...
      {tab === 'portfolio' && (
        <div>
          <div style={{ padding: '14px 16px 8px', borderBottom: '1px solid var(--border)', fontSize: 14, fontWeight: 500, display: 'flex', justifyContent: 'space-between' }}>
            Projects
          </div>
          {portfolio.length === 0
            ? <p style={{ padding: 24, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>No portfolio yet.</p>
            : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8, padding: 12 }}>
                {portfolio.map(p => (
                  <div key={p.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                    <div style={{ height: 90, background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>
                    ...
                    <div style={{ padding: '7px 9px' }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 1 }}>{p.area} · {p.project_year}</div>
                    </div>
                  </div>
                ))}
              </div>}
      ...
      {tab === 'team' && (
        <div>
          <div style={{ background: 'var(--primary-light)', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ padding: '0 16px' }}>
            {employees.length === 0
              ? <p style={{ padding: 24, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>No team profiles yet.</p>
          ...
          <div style={{ padding: 16 }}>
            <button onClick={() => navigate('register-employee', { company })} style={{
              ...
              borderRadius: 20, background: '#fff', color: 'var(--text2)',
              fontSize: 13, cursor: 'pointer'
  ...
  return (
    <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
        <span style={{ fontSize: 13, fontWeight: 500 }}>{review.reviewer_name || 'Anonymous'}</span>
        <span style={{ fontSize: 11, color: 'var(--text3)' }}>{new Date(review.created_at).toLocaleDateString('en-AE', { month: 'short', year: 'numeric' })}</span>
      </div>
      <div style={{ color: 'var(--amber)', fontSize: 12, marginBottom: 5 }}>
        {'★'.repeat(review.rating)}{'☆'.repeat(5-review.rating)}
        <span style={{ fontSize: 11, color: 'var(--text3)', marginLeft: 4 }}>{review.rating}.0</span>
      </div>
      <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.5 }}>{review.review_text}</div>
...
function EmployeeCard({ emp, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      padding: 12, marginTop: 10, cursor: 'pointer'
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 1 }}>{emp.designation}</div>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 500 }}>{emp.avg_rating || '0.0'}</span>
        <span style={{ fontSize: 11, color: 'var(--text3)' }}>({emp.total_reviews || 0} reviews)</span>
      </div>
      {emp.is_verified && (
