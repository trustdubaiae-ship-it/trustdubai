...
export default function CompanyCard({ company, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 2 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: plan === 'platinum' ? '#f1f5f9' : 'var(--text-primary)' }}>
              {company.name || company.company_name}
            </div>
            <PlanBadge plan={plan} />
          </div>
          <div style={{ fontSize: 11, color: plan === 'platinum' ? '#a78bfa' : 'var(--text-secondary)' }}>{company.category}</div>
        </div>
        {company.is_verified && (
          <span style={{ background: plan === 'platinum' ? 'rgba(16,185,129,0.2)' : 'var(--verified-bg)', color: plan === 'platinum' ? '#34d399' : 'var(--verified-text)', fontSize: 10, padding: '2px 7px', borderRadius: 10, whiteSpace: 'nowrap', flexShrink: 0, marginLeft: 6 }}>
            ✓ Verified
          </span>
        )}
      </div>
      ...
      {/* Location */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: plan === 'platinum' ? '#94a3b8' : 'var(--text-secondary)' }}>
        <i className="ti ti-map-pin" style={{ fontSize: 11 }} />
        {company.area || company.location}
        {(company.employee_count > 0) && <span> · {company.employee_count} employees</span>}
      </div>
